...
void render(struct game *g)
{
	...
	double num_tiles_w = GRID_NORMAL_SIZE * 100.0 / g->zoom;
	double num_tiles_h = GRID_NORMAL_SIZE * 100.0 / g->zoom;
	...
	for (i = 0, sx = (g->width - tile_size_x) / 2; i < (num_tiles_w + 1)/ 2; i++, sx += tile_size_x) {
		for (j = 0, sy = (g->height - tile_size_y) / 2; j < (num_tiles_h + 1) / 2; j++, sy += tile_size_y)
			draw_rect(g->renderer, g->grid[g->cx + i][g->cy + j], sx-1, sy-1, round(tile_size_x)+2, round(tile_size_y)+2);
		for (j = 0, sy = (g->height - tile_size_y) / 2; j < (num_tiles_h + 1) / 2; j++, sy -= tile_size_y)
			draw_rect(g->renderer, g->grid[g->cx + i][g->cy - j], sx-1, sy-1, round(tile_size_x)+2, round(tile_size_y)+2);
	}
	for (i = 0, sx = (g->width - tile_size_x) / 2; i < (num_tiles_w + 1)/ 2; i++, sx -= tile_size_x) {
